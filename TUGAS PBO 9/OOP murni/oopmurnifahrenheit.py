...
class FrmFahrenheit:
    ...
    def onHitung(self):
        # Suhu dalam Celsius
        suhu_fahrenheit = self.txtFahrenheit.get()
        C = self.get_celsius(float(suhu_fahrenheit))
        self.txtCelsius.delete(0, END)
        self.txtCelsius.insert(END, str(C))

        # Suhu dalam Reamur
        R = self.get_reamur(float(suhu_fahrenheit))
        self.txtReamur.delete(0, END)
        self.txtReamur.insert(END, str(R))

        # Suhu dalam Kelvin
        K = self.get_kelvin(float(suhu_fahrenheit))
