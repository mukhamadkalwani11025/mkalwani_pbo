...
class perawat:
    ...
    def simpan(self):
        self.conn = mydb()
        val = (self.__nip, self.__nama, self.__jk, self.__spesialis)
        sql="INSERT INTO data_perawat (nip, nama, jk, spesialis) VALUES " + str(val)
    ...
    def update(self, id):
        self.conn = mydb()
        val = (self.__nip, self.__nama, self.__jk, self.__spesialis, id)
        sql="UPDATE data_perawat SET nip = %s, nama = %s, jk=%s, spesialis=%s WHERE id=%s"
    ...
    def updateByNIP(self, nip):
        self.conn = mydb()
        val = (self.__nama, self.__jk, self.__spesialis, nip)
        sql="UPDATE data_perawat SET nama = %s, jk=%s, spesialis=%s WHERE nip=%s"
        self.affected = self.conn.update(sql, val)
        self.conn.disconnect
        return self.affected        
